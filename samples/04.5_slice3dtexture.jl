...
void main()
{
    ...
    TexCoord = vec3(aTextureCoordinate.x, aTextureCoordinate.y, aTextureCoordinate.z);
}
"""
...
function whichslice(timeofstart, timenow)
    ...
    # The vertex Z coordinate is in the range [-1, 1], but we want [-0.5, 0.5].
    # The texture R coordinate (corresponds to the Z coordinate) is in the range [0, 1].
    (v / 2f0, v / 2f0 + 0.5f0)
end
...
function run()
    ...
    window = GLFW.CreateWindow(camera.windowwidth, camera.windowheight, "Alfar Sample 04.5: One moving slice, other viewing angle")
    ...
    while !GLFW.WindowShouldClose(window)
        ...
        # Calculate the vertex Z coordinate and the texture R coordinate for
        # this frame.
        timenow = time()
        vertexz, texr = whichslice(timeofstart, timenow)

        # Create a new geometry every frame.
